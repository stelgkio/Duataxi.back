...
namespace DuaTaxi.AuthServer.Controllers
{
    public class BaseController : Controller
    {
        ...
        {
            // Kanonika to SendAsync xrhsimopoihte mia fora gia na steilei 1 mnm .Emeis theloume na steiloume polla kserodas oti einai sto idio operation.
            if (!Response.Headers.ContainsKey("X-Operation")){
        ...
        protected Guid UserId
            => string.IsNullOrWhiteSpace(User?.Identity?.Name) ?
               Guid.NewGuid() : Guid.Parse(User.Claims.Where(x => x.Type.Contains("nameidentifier")).Select(x => x.Value).FirstOrDefault());
