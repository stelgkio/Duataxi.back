...
namespace DuaTaxi.Common.Mvc
{
    public static class Extensions
    {
        ...
        public static T BindId<T>(this T model, Expression<Func<T, string>> expression)
            => model.Bind<T, string>(expression, Guid.NewGuid().ToString());
            ...
            object value)
        {
            ...
            var field = modelType.GetFields(BindingFlags.Instance | BindingFlags.NonPublic);
              ...
              var set= field.FirstOrDefault(x => x.Name.ToLowerInvariant().StartsWith($"<{propertyName}>"));
            if (field == null)
            {
                return model;
            }
