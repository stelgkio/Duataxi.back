...
namespace DuaTaxi.CoreApi.Controllers
{
    ...
    public abstract class BaseController : ControllerBase
    {
        ...
        protected IActionResult Single<T>(T model, Func<T, bool> criteria = null)
        {
            ...
            if (model == null || HttpContext.Response.StatusCode == 400)
            {
                return BadRequest();
            }
        ...
        protected IActionResult Collection<T>(PagedResult<T> pagedResult, Func<PagedResult<T>, bool> criteria = null)
        {
            ...
            if (pagedResult == null || HttpContext.Response.StatusCode==400)
            {
                return BadRequest();
            }
        ...
        protected Guid UserId
            => string.IsNullOrWhiteSpace(User?.Identity?.Name) ?
               Guid.Empty : Guid.Parse(User.Claims.Where(x => x.Type.Contains("nameidentifier")).Select(x => x.Value).FirstOrDefault());
